//
//  Custom event listener that listens for pushes
//
var storage = []
...
function packetConcat(arr){
  ...
  if(p_concat[0] == '03' && p_concat[p_bytes - 1] == '40'){
	var data = packetParser(p)
	addItem(data)
	// if storage is cleared by assignment (storage = []),
	// it assigns new array which doesn't contain the custom push listener
...
function getPayload(hexArr, bytes) {
  ...
  // wrong logic.  this will return the first instance of that value
